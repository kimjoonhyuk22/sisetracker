...
def scrape_weather():
    ...
    morning_rain_rate = soup.find("span", attrs={"class":"weather_left"}).get_text().strip() # 오전 강수확률
    # ppp2 = soup.find_all("span", attrs={"class":"weather_left"}) 
    # afternoon_rain_rate = ppp2[1].get_text()  28,29 2줄을 30한줄로 줄일수 있음!
    afternoon_rain_rate = soup.find_all("span", attrs={"class":"weather_left"})[1].get_text()
    #afternoon_rain_rate = soup.find("span", attrs={"class":"weather_left"}).get_text().strip() # 오후 강수확률

    # # 미세먼지 OO㎍/㎥좋음
    # # 초미세먼지 OO㎍/㎥좋음
    dust = soup.find("div", attrs={"class":"report_card_wrap"})
    pm10 = dust.find_all("span", attrs={"class":"txt"})[0].get_text() # 미세먼지
    pm25 = dust.find_all("span", attrs={"class":"txt"})[1].get_text() # 초미세먼지  
...
#     soup = create_soup(url)
#     news_list = soup.find("ul", attrs={"class":"cc_text_list"}) #.find_all("li", limit=3)
#     print(news_list)
